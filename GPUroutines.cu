...
__global__ void GPUcrossover(Gate * population, Gate *temp_population, float *fitness, curandState *globalState,
	...
	while (tid < sizeOfPopulation) {
		...
		for (i = 0; i < p1; i++) {
			temp_population[tid * sizeOfChromosome + i] = population[iof * sizeOfChromosome + i];

		}
		for (i = p1; i < p2; i++) {
			temp_population[tid * sizeOfChromosome + i] = population[ios * sizeOfChromosome + i];
...
__global__ void generatePopulationDev(Gate * g, curandState *globalState) {
	...
	while (tid < sizeOfChromosome * sizeOfPopulation) {
		...
		int r;
		double parameter;
		int k = 1 + ((int) ((curand_uniform_double(&state) * scaleForRandom))) % (mutationRange);
		//k*=2;
		parameter = PI / k;
		int sign = ((int) ((curand_uniform_double(&state) * scaleForRandom))) % 2;
		if (sign == 0)
		...
		if (numberOfWires > 1) {
			if (numberOfWires > 2)
				r = 1 + ((int) (curand_uniform_double(&state) * scaleForRandom)) % 8;
			else
				r = 1 + ((int) (curand_uniform_double(&state) * scaleForRandom)) % 3;
			if (r == 1 || r == 3 || r == 5 || r == 4 || r == 6 || r == 7) {
				g[i].index1 = 1 + ((int) (curand_uniform_double(&state) * scaleForRandom)) % numberOfWires;
				g[i].index2 = g[i].index1;
				g[i].direction = ((int) (curand_uniform_double(&state) * scaleForRandom)) % 3;
			}
			if (r == 2) {
				int ri1;
				if (numberOfWires == 2) {
					...
					ri1 = 1 + ((int) (curand_uniform_double(&state) * scaleForRandom)) % (numberOfWires - 1);
				}
				int ri2 = ri1 + 1;
				g[i].index1 = ri1;
				g[i].index2 = ri2;
			...
			if (mutationp < 0.5) {
				...
				if (numberOfWires >= 2) {
					if (numberOfWires > 2)
						...
						r = 1 + ((int) (curand_uniform_double(&state) * scaleForRandom)) % 2;
					double parameter;
					int k = 1 + ((int) ((curand_uniform_double(&state) * scaleForRandom))) % (mutationRange);
					parameter = PI / k;
					int sign = ((int) (curand_uniform_double(&state) * scaleForRandom)) % 2;
				...
				if (comparecuDoubleComplex(g[i].parameter, k) != 1) {
					...
					r = ((int) (curand_uniform_double(&state) * scaleForRandom)) % 2;
					if (r < 1) {
						//	printf("Here\n");
						parameter += delta;
						if (parameter > PI)
							parameter -= 2 * PI;
						cuDoubleComplex temp = make_cuDoubleComplex(parameter, 0);
						g[i].parameter = temp;
	...
	while (tid < sizeOfChromosome * sizeOfPopulation) {
		...
		if (g.index1 == g.index2) {
			...
			case X:
				...
				memory[tid * 4 * 4 + 0] = cuCcos(cuCdiv(g.parameter, make_cuDoubleComplex(2, 0)));
				memory[tid * 4 * 4 + 1] = cuCmul(make_cuDoubleComplex(0, -1),
						cuCsin(cuCdiv(g.parameter, make_cuDoubleComplex(2, 0))));
				memory[tid * 4 * 4 + 2] = cuCmul(make_cuDoubleComplex(0, -1),
						cuCsin(cuCdiv(g.parameter, make_cuDoubleComplex(2, 0))));
				memory[tid * 4 * 4 + 3] = cuCcos(cuCdiv(g.parameter, make_cuDoubleComplex(2, 0)));
				break;
			case Y:

				memory[tid * 4 * 4 + 0] = cuCcos(cuCdiv(g.parameter, make_cuDoubleComplex(2, 0)));
				memory[tid * 4 * 4 + 1] = cuCsin(cuCdiv(g.parameter, make_cuDoubleComplex(-2, 0)));
				memory[tid * 4 * 4 + 2] = cuCsin(cuCdiv(g.parameter, make_cuDoubleComplex(2, 0)));
				memory[tid * 4 * 4 + 3] = cuCcos(cuCdiv(g.parameter, make_cuDoubleComplex(2, 0)));
				break;
			...
			case Z:
				...
				memory[tid * 4 * 4 + 0] = cuCexp(
						cuCmul(make_cuDoubleComplex(0, -1), cuCdiv(g.parameter, make_cuDoubleComplex(2, 0))));
		...
		if (abs(g.index1 - g.index2) == 1 && comparecuDoubleComplex(g.parameter, k) == 0) {
			memory[tid * 4 * 4 + 0] = make_cuDoubleComplex(1, 0);
			memory[tid * 4 * 4 + 5] = cuCexp(cuCmul(g.parameter, make_cuDoubleComplex(0, 1)));
			memory[tid * 4 * 4 + 10] = cuCexp(cuCmul(g.parameter, make_cuDoubleComplex(0, 1)));
			memory[tid * 4 * 4 + 15] = make_cuDoubleComplex(1, 0);
			for (i = 0; i < 4 * 4; i++) {
				memory[tid * 4 * 4 + i] = cuCmul(memory[tid * 4 * 4 + i],
						cuCexp(cuCmul(make_cuDoubleComplex(0, -1), cuCdiv(g.parameter, make_cuDoubleComplex(2, 0)))));
			}
		}
		//indexes differing more than by one mean there should be swap gates
		if (abs(g.index1 - g.index2) == 1 && comparecuDoubleComplex(g.parameter, k) == 1) {
			memory[tid * 4 * 4 + 0] = make_cuDoubleComplex(1, 0);
...
__global__ void prepareMatrices(Gate *G, cuDoubleComplex * sourcememory, cuDoubleComplex * tempmemory,
	...
	while (tid < sizeOfChromosome * sizeOfPopulation) {
		...
		for (i = 0; i < size1; i++) {
			...
			tempmemory[tid * size * size + i * (size1) + i] = make_cuDoubleComplex(1, 0);
		}
		if (g.index1 == g.index2) {
		...
		for (i = 0; i < size2; i++) {
			tempmemory[tid * size * size + i * (size2) + i] = make_cuDoubleComplex(1, 0);
		}
		if (g.index1 == g.index2) {
...
void gpu_blas_mmul(cuDoubleComplex* temp_circuit, cuDoubleComplex* final_circuit, cuDoubleComplex* Gates, const int m,
		const int k, const int n, cublasHandle_t handle, int size, cudaStream_t * streams) {
	...
	for (i = 1; i < sizeOfChromosome; i++) {
		...
		for (j = 0; j < sizeOfPopulation; j++) {
			...
			cublasZgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, m, n, k, alpha,
					Gates + size * size * j * sizeOfChromosome + i * size * size, lda, final_circuit + size * size * j,
...
__global__ void evaluateFitnessGPU(cuDoubleComplex * Data, cuDoubleComplex * Target, float *tempfitness, int size) {
	int tid = threadIdx.x + blockIdx.x * blockDim.x;
	while (tid < sizeOfPopulation * size * size) {
		atomicAdd(tempfitness + tid / size / size,
				(float) ((cuCabs(Data[tid]) - cuCabs(Target[tid % (size * size)]))
						* (cuCabs(Data[tid]) - cuCabs(Target[tid % (size * size)]))));
		tid += blockDim.x * gridDim.x;
	}
